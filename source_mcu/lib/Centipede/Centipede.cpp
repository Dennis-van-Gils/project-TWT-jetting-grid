...
void Centipede::WriteRegisterPin(int port, int regpin, int subregister,
                                 int level) {
...
int Centipede::digitalRead(int pin) {
  ...
  int returnval =
      (CSDataArray[0] >> (pin - ((port << 1) + subregister) * 8)) & 1;
...
void Centipede::portInterrupts(int port, int gpintval, int defval,
                               int intconval) {
